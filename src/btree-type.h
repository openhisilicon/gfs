...
static inline void *BTREE_FN(lookup_le)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	unsigned long _key = key;
	return btree_lookup_le(&head->h, BTREE_TYPE_GEO, &_key);
}
...
static inline void *BTREE_FN(first)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE *key)
{
	unsigned long _key;
	void *val = btree_first(&head->h, BTREE_TYPE_GEO, &_key);
	if (val)
		*key = _key;
	return val;
}
...
static inline void *BTREE_FN(lookup_le)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE key)
{
	return btree_lookup_le(&head->h, BTREE_TYPE_GEO, (unsigned long *)&key);
}
...
static inline void *BTREE_FN(first)(BTREE_TYPE_HEAD *head, BTREE_KEYTYPE *key)
{
	return btree_first(&head->h, BTREE_TYPE_GEO, (unsigned long *)key);
}
